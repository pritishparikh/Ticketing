    ...
    public class PriorityController : ControllerBase
    {
        ...
        public ResponseModel GetPriorityList(int PriorityFor=1)
        {
            ...
            try
            {
                ...
                objPriority = newMasterSubCat.GetPriorityList(new PriorityService(Cache, Db), authenticate.TenantId, PriorityFor);
        ...
        public ResponseModel AddPriority(string PriorityName, int status,int PriorityFor=1)
        {
            ...
            try
            {
                ...
                Authenticate authenticate = new Authenticate();
                authenticate = SecurityService.GetAuthenticateDataFromTokenCache(Cache, SecurityService.DecryptStringAES(token));
                int result = masterCaller.Addpriority(new PriorityService(Cache, Db), PriorityName, status, authenticate.TenantId, authenticate.UserMasterID, PriorityFor);
                statusCode =
                result == 0 ?
                       (int)EnumMaster.StatusCode.RecordNotFound : (int)EnumMaster.StatusCode.Success;
                statusMessage = CommonFunction.GetEnumDescription((EnumMaster.StatusCode)statusCode);
        ...
        public ResponseModel  UpdatePriority(int PriorityID, string PriorityName, int status, int PriorityFor=1)
        {
            ...
            try
            {
                ...
                Authenticate authenticate = new Authenticate();
                authenticate = SecurityService.GetAuthenticateDataFromTokenCache(Cache, SecurityService.DecryptStringAES(token));
                int result = masterCaller.Updatepriority(new PriorityService(Cache, Db), PriorityID, PriorityName, status, authenticate.TenantId, authenticate.UserMasterID, PriorityFor);
                statusCode =
                result == 0 ?
                       (int)EnumMaster.StatusCode.RecordNotFound : (int)EnumMaster.StatusCode.Success;
                statusMessage = CommonFunction.GetEnumDescription((EnumMaster.StatusCode)statusCode);
        ...
        public ResponseModel PriorityList(int PriorityFor)
        {
            ...
            try
            {
                ...
                objPriority = newMasterSubCat.PriorityList(new PriorityService(Cache, Db), authenticate.TenantId, PriorityFor);
