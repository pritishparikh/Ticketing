...
namespace Easyrewardz_TicketSystem.Services
{
    public class StoreUserService:IStoreUser
    {
        ...
        /// <param name="UserID"></param>
        /// <returns></returns>
        public List<StoreUserListing> GetStoreReportUserList(int tenantID, string RegionID, string ZoneID, int UserID)
        {
            DataSet ds = new DataSet();
            MySqlCommand cmd = new MySqlCommand();
            List<StoreUserListing> UsermasterList = new List<StoreUserListing>();
            try
            {
                ...
                cmd1.CommandType = CommandType.StoredProcedure;
                cmd1.Parameters.AddWithValue("@_tenantID", tenantID);
                cmd1.Parameters.AddWithValue("@_RegionID", string.IsNullOrEmpty(RegionID) ? "" : RegionID.TrimEnd(','));
                cmd1.Parameters.AddWithValue("@_ZoneID", string.IsNullOrEmpty(ZoneID) ? "" : ZoneID.TrimEnd(',') );
                cmd1.Parameters.AddWithValue("@_UserID", UserID);
                MySqlDataAdapter da = new MySqlDataAdapter();
                da.SelectCommand = cmd1;
                da.Fill(ds);
