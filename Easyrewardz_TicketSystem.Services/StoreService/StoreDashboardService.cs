...
namespace Easyrewardz_TicketSystem.Services
{
    public class StoreDashboardService : IStoreDashboard
    {
            ...
            try
            {
                ...
                if (ds != null && ds.Tables[0] != null)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        StoreDashboard.ModifyBy = ds.Tables[0].Rows[i]["ModifyBy"] == DBNull.Value ? string.Empty : Convert.ToString(ds.Tables[0].Rows[i]["ModifyBy"]);
        ...
        /// <param name="tenantID"></param>>
        /// <returns></returns>
        public async Task<List<GetStoreByTenantModel>> GetStoreByTenantID(int tenantID, int userID)
        {
            MySqlCommand cmd = new MySqlCommand();
            List<GetStoreByTenantModel> lstGetStoreByTenantModel = new List<GetStoreByTenantModel>();
            try
            {
                ...
                using (conn)
                {
                    cmd = new MySqlCommand("SP_GetStoreByTenantID", conn);
                    cmd.Connection = conn;
            ...
            return lstGetStoreByTenantModel;
        }
        ...
        /// <param name="tenantID"></param>>
        /// <param name="userID"></param>>
        /// <param name="ReportDataRequest"></param>>
        /// <returns></returns>
        public async Task<List<StoreUserProductivityReport>> StoreUserProductivityReport(int tenantID, int userID, ReportDataRequest reportDataRequest)
        {
            MySqlCommand cmd = new MySqlCommand();
            ...
            try
            {
                ...
                using (conn)
                {
                    cmd = new MySqlCommand("Sp_GetStoreUserProductivityReport", conn);
                    cmd.Connection = conn;
                    cmd.Parameters.AddWithValue("@Tenant_ID", tenantID);
                    cmd.Parameters.AddWithValue("@User_ID", userID);
                    cmd.Parameters.AddWithValue("@_UserIds", string.IsNullOrEmpty(reportDataRequest.UserIds) ? "" : reportDataRequest.UserIds.TrimEnd(','));
                    cmd.Parameters.AddWithValue("@_BrandIDs", string.IsNullOrEmpty(reportDataRequest.BrandIDs) ? "" : reportDataRequest.BrandIDs.TrimEnd(','));
                    cmd.Parameters.AddWithValue("@_StoreIDs", string.IsNullOrEmpty(reportDataRequest.StoreIDs) ? "" : reportDataRequest.StoreIDs.TrimEnd(','));
                    cmd.Parameters.AddWithValue("@_ZoneIDs", string.IsNullOrEmpty(reportDataRequest.ZoneIDs) ? "" : reportDataRequest.ZoneIDs.TrimEnd(','));
                    cmd.Parameters.AddWithValue("@_DateFrom", reportDataRequest.DateFrom);
                    cmd.Parameters.AddWithValue("@_DateEnd", reportDataRequest.DateEnd);
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    using (var dr = await cmd.ExecuteReaderAsync())
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            StoreUserProductivityReport obj = new StoreUserProductivityReport()
                            {
                                UserID = Convert.ToInt32(dr["UserID"]),
                                UserName = dr["UsersName"] == DBNull.Value ? string.Empty : Convert.ToString(dr["UsersName"]),
                                StoreName = dr["StoreName"] == DBNull.Value ? string.Empty : Convert.ToString(dr["StoreName"]),
                                ...
                                CampaignAssigned = dr["CampaignAssigned"] == DBNull.Value ? 0 : Convert.ToInt32(dr["CampaignAssigned"]),
                                CustomerCount = dr["CustomerCount"] == DBNull.Value ? 0 : Convert.ToInt32(dr["CustomerCount"]),
                                Contacted = dr["Contacted"] == DBNull.Value ? 0 : Convert.ToInt32(dr["Contacted"]),
                                NotContacted = dr["NotContacted"] == DBNull.Value ? 0 : Convert.ToInt32(dr["NotContacted"]),
                                FollowUp = dr["FollowUp"] == DBNull.Value ? 0 : Convert.ToInt32(dr["FollowUp"]),
                            };
                            lstStoreUserProductivityReport.Add(obj);
                        }
                    }
                }
            }
            ...
            return lstStoreUserProductivityReport;
        }
        ...
        /// <param name="userID"></param>>
        /// <returns></returns>
        public async Task<List<StoreProductivityReport>> StoreProductivityReport(int tenantID, int userID, ReportDataRequest reportDataRequest)
        {
            MySqlCommand cmd = new MySqlCommand();
            ...
            try
            {
                ...
                using (conn)
                {
                    cmd = new MySqlCommand("Sp_StoreProductivityReport", conn);
                    cmd.Connection = conn;
                    cmd.Parameters.AddWithValue("@Tenant_ID", tenantID);
                    cmd.Parameters.AddWithValue("@User_ID", userID);
                    cmd.Parameters.AddWithValue("@_UserIds", string.IsNullOrEmpty(reportDataRequest.UserIds) ? "" : reportDataRequest.UserIds.TrimEnd(','));
                    ...
                    using (var dr = await cmd.ExecuteReaderAsync())
                    {
                        ...
                        while (dr.Read())
                        {
                            ...
                            StoreProductivityReport obj = new StoreProductivityReport()
                            {
                                StoreID = Convert.ToInt32(dr["StoreID"]),
                                StoreName = dr["StoreName"] == DBNull.Value ? string.Empty : Convert.ToString(dr["StoreName"]),
                                CampaignAssigned = dr["CampaignAssigned"] == DBNull.Value ? 0 : Convert.ToInt32(dr["CampaignAssigned"]),
                            ...
                            lstStoreProductivityReport.Add(obj);
                        }
                    }
                }
            }
            ...
            return lstStoreProductivityReport;
        }
    }
}
