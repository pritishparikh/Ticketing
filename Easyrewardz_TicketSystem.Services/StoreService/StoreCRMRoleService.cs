...
namespace Easyrewardz_TicketSystem.Services
{
    public class StoreCRMRoleService : IStoreCRMRole
    {
        ...
        public StoreCRMRoleService(string _connectionString)
        {
        ...
        public int InsertUpdateStoreCRMRole(int CRMRoleID, int tenantID, string RoleName, bool RoleisActive, int UserID, string ModulesEnabled, string ModulesDisabled)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand(CRMRoleID > 0 ? "SP_UpdateStoreCRMRole" : "SP_InsertStoreCRMRole", conn);
                cmd.Connection = conn;
        ...
        /// <param name="tenantID"></param>
        /// <param name="CRMRoleID"></param>
        public int DeleteStoreCRMRole(int tenantID, int CRMRoleID)
        {
            int deletecount = 0;
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SP_DeleteStoreCRMRole", conn);
                cmd.Connection = conn;
        ...
        /// </summary>
        /// <param name="tenantID"></param>
        public List<StoreCRMRoleModel> GetStoreCRMRoleList(int tenantID)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd1 = new MySqlCommand("SP_GetStoreCRMRolesDetails", conn);
                ...
                if (ds != null && ds.Tables != null)
                {
                    ...
                    {
                        objCRMLst = ds.Tables[0].AsEnumerable().Select(r => new StoreCRMRoleModel()
                        {
                        ...
                        if (ds.Tables[1] != null && ds.Tables[1].Rows.Count > 0)
                        {
                            for (int i = 0; i < objCRMLst.Count; i++)
                            {
                                objCRMLst[i].Modules = ds.Tables[1].AsEnumerable().Where(r => r.Field<object>("CRMRolesID") != System.DBNull.Value &&
                                    objCRMLst[i].CRMRoleID == Convert.ToInt32(r.Field<object>("CRMRolesID"))).Select(r => new StoreModuleDetails()
                                    {
                                        CRMRoleID = Convert.ToInt32(r.Field<object>("CRMRolesID")),
        ...
        public List<string> StoreBulkUploadCRMRole(int TenantID, int CreatedBy, int RoleFor, DataSet DataSetCSV)
        {
            ...
            try
            {
                if (DataSetCSV != null && DataSetCSV.Tables.Count > 0)
                {
                    if (DataSetCSV.Tables[0] != null && DataSetCSV.Tables[0].Rows.Count > 0)
                    {
                        ...
                        conn.Open();
                        MySqlCommand cmd = new MySqlCommand("SP_BulkUploadStoreCRMRoles", conn);
                        cmd.Connection = conn;
