...
namespace Easyrewardz_TicketSystem.Services
{
    public partial class AppointmentServices : IAppointment
    {
        ...
        public async Task<SlotFilterModel> GetSlotFilterDetails(int TenantID, string ProgramCode)
        {
            DataTable StoreDt = new DataTable();
            DataTable OpDaysDt = new DataTable();
            DataTable SlotTemplatesDt = new DataTable();
            SlotFilterModel SlotFilter = new SlotFilterModel();

            List<SlotStoreFilter> SlotStores = new List<SlotStoreFilter>();
            List<OperationalDaysFilter> OperationalDays = new List<OperationalDaysFilter>();
            List<SlotTemplateModel> SlotTemplate = new List<SlotTemplateModel>();
            try
            {
                if (conn != null && conn.State == ConnectionState.Closed)
                {
                ...
                using (conn)
                {
                    MySqlCommand cmd = new MySqlCommand("SP_HSGetSlotFilters", conn)
                    {
                        CommandType = CommandType.StoredProcedure
                    };
                    cmd.Parameters.AddWithValue("@_TenantId", TenantID);
                    ...
                    using (var reader = await cmd.ExecuteReaderAsync())
                    {
                        ...
                        {
                            StoreDt.Load(reader);
                            OpDaysDt.Load(reader);
                            SlotTemplatesDt.Load(reader);

                        }

                        if (StoreDt != null && StoreDt.Rows.Count > 0)
                        {
                            foreach (DataRow dr in StoreDt.Rows)
                            {
                                SlotStores.Add(new SlotStoreFilter() {
                            ...
                            SlotFilter.SlotStores = SlotStores;
                        }

                        if (OpDaysDt != null && OpDaysDt.Rows.Count > 0)
                        {
                            foreach (DataRow dr in OpDaysDt.Rows)
                            {
                                OperationalDays.Add(new OperationalDaysFilter()
                                {

                                    DayIDs = dr["opid"] == DBNull.Value ? string.Empty : Convert.ToString(dr["opid"]),
                                    DayNames = dr["opdays"] == DBNull.Value ? string.Empty : Convert.ToString(dr["opdays"])

                                });
                            }
                            SlotFilter.OperationalDays = OperationalDays;
                        }

                        if (SlotTemplatesDt != null && SlotTemplatesDt.Rows.Count > 0)
                        {
                            foreach (DataRow dr in SlotTemplatesDt.Rows)
                            {
                                SlotTemplate.Add(new SlotTemplateModel()
                                {
                            ...
                            SlotFilter.SlotTemplate = SlotTemplate;
                        }
            ...
            finally
            {
                ...
                if (SlotTemplatesDt != null)
                {
                    SlotTemplatesDt.Dispose();
                }
            }
